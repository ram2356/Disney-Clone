...
function AISuggestion({term}: {term: string}) {
  ...
  const generateText = () => {
    if (isLoading || isValidating)
      return (
        <>
            <div className="animation-spin rounded-full h-10 w-10 border-b-2 border-white" />
    ...
    return (
        <>
            <div className="animate-pulse rounded-full bg-gradient-to-t from-white h-10 w-10 border-2 flex-shrink-0
            border-white"/>
